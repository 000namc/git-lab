...
4. filter-repo: `git log --all --oneline -- secrets.env` 로 아직 히스토리에 있는지 확인. `git filter-repo --path secrets.env --invert-paths --force` 후 다시 확인. 어떤 커밋의 해시가 바뀌고 어떤 커밋은 그대로인가. 빈 커밋이 된 것은 어떻게 되나. (`--force` 는 fresh clone 이 아닐 때 필요)
